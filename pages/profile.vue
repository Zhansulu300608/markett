  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <main class="max-w-7xl mx-auto px-4 flex gap-8 py-10">
      ...
      <aside class="w-72 bg-white rounded-2xl shadow-sm p-6">
        ...
        <span class="flex justify-center px-4 py-1 text-xs font-semibold text-orange-600 bg-orange-100 rounded-full mb-6">
          Покупатель
        </span>

        <div class="w-full mb-6 rounded-xl border bg-gray-50 px-4 py-3 flex justify-between items-center">
          <div class="text-xs text-gray-500">Всего заказов</div>
          <div class="text-2xl font-semibold">{{ ordersCount }}</div>
        </div>

        <nav class="space-y-2">
          <NuxtLink to="/profile" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-100 font-medium">Профиль</NuxtLink>
          <NuxtLink to="/order" class="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-gray-100">Мои заказы</NuxtLink>
        </nav>
      </aside>
      ...
      <section class="flex-1">
        ...
        <form @submit.prevent="onSubmit" class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl">
          <div>
